...
@section('title', Auth::User()->cPanel->TCpanel_AppName.' | Edit Appointment')

@section('content_header', 'Edit Appointment')
...
<?php
	...
	$tgltransaksi 	= new DateTime($items->TJanjiJalan_TglJanji);
	$tgllahir 		= new DateTime($items->TJanjiJalan_TglLahir);
...
<div class="row font-medium">
	...
	<form class="form-horizontal form-label-left" action="/appointment/{{$items->id}}" id="appointment" method="post" data-parsley-validate onsubmit="return checkFormAppointment()">
		    ...
		    <div class="box-body">
		    	...
		    	<div class="form-group">
                	...
                	<div class="col-md-9 col-sm-9 col-xs-12">
                		<input type="text" name="nojan" id="nojan" class="form-control col-md-7 col-xs-12" value="" readonly>
                	</div>
                </div>
		    	...
		    	<div class="form-group">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<div class="input-group date">
							...
							<input type="text" name="tgljanji" id="datepicker2" class="form-control has-feedback-left" value="{{ date_format($tgltransaksi, 'm/d/Y')}}" onchange="checkTglJanji();">
	                    </div>
	                </div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<div class="input-group">
	                    	...
	                    	<input type="text" name="jamjanji" id="jamjanji" class="form-control" value="{{ date_format($tgltransaksi, 'H:i')}}">
		                </div>
					</div>
				</div>
...
<script type="text/javascript">
    ...
    function fillData(){
    	...
    	$('#nojan').val('{{$items->TJanjiJalan_NoJan}}');
    	$('#pasiennorm').val('{{$items->TPasien_NomorRM}}');
    	$('#nama').val('{{$items->TJanjiJalan_Nama}}');
    	$('#jk').val('{{$items->TJanjiJalan_Gender}}');
    	$('#datepicker').val('<?php echo date_format($tgllahir, 'm/d/Y'); ?>');
    	$('#pasienumurthn').val('{{$items->TJanjiJalan_PasienUmurThn}}');
    	$('#pasienumurbln').val('{{$items->TJanjiJalan_PasienUmurBln}}');
    	$('#pasienumurhari').val('{{$items->TJanjiJalan_PasienUmurHr}}');
    	$('#telepon').val('{{$items->TJanjiJalan_PasienTelp}}');
    	$('#alamat').val('{{$items->TJanjiJalan_Alamat}}');
    	$('#datepicker2').val('<?php echo date_format($tgltransaksi, 'm/d/Y'); ?>');
    	$('#jamjanji').val('{{$items->TJanjiJalan_JamJanji}}');
    	$('#unit').val('{{$items->TUnit_Kode}}');
    	$('#pelaku').val('{{$items->TPelaku_Kode}}');
    	$('#keterangan').val('{{$items->TJanjiJalan_Keterangan}}');

    	if('{{$items->TJanjiJalan_Gender}}' == 'L'){
