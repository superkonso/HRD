...
class VerifbelilogController extends Controller
{
    ...
    public function store(Request $request)    {
        ...
        //                 ->where('TTrimaLog_Nomor','=',$request->terimanomor)
        //                 ->update(['TTrimaLog_ReffNo' => (is_null($request->nofaktur) ? '' : $request->nofaktur), 'TTrimaLog_LPBNo' => (is_null($request->nolpb) ? '' : $request->nolpb),'TTrimaLog_Status' => '1']);
        ...
        $logbook->TLogBook_LogMenuNo       = '13005';
    ...
    public function update(Request $request, $id)    {
        ...
        $jurnal_lama    = Jurnal::where('TJurnal_Nomor', '=', $request->terimanomor)->first(); 
        $itemjurnal     = json_decode($request->arrItem);
        ...
        // Delete Jurnal Lama
        \DB::table('tjurnal')->where('TJurnal_Nomor', '=', $request->terimanomor)->delete();
        ...
        // ========================= simpan ke tlogbook ==============================
            ...
            $logbook->TLogBook_LogIPAddress     = $ip;
            $logbook->TLogBook_LogDate          = date('Y-m-d H:i:s');
            $logbook->TLogBook_LogMenuNo        = '13304';
            $logbook->TLogBook_LogMenuNama      = url()->current();
            $logbook->TLogBook_LogJenis         = 'E';
            $logbook->TLogBook_LogNoBukti       = $request->terimanomor;
            $logbook->TLogBook_LogKeterangan    = 'Update Jurnal Penerimaan Logistik : '.$request->terimanomor;
            $logbook->TLogBook_LogJumlah        = floatval($totDebet);
