...
class TindakanmedisinapsController extends Controller
{
    public function __construct()
    {
        $this->middleware('MenuLevelCheck:04,003');
    }
    ...
    public function index()
    {
        ...
        return view::make('Rawatinap.Tindakanmedis.view', compact('autoNumber', 'units', 'pelakus', 'admvars', 'tarifvars', 'provinsi', 'prsh'));             
    }
    ...
    public function store(Request $request)
    {
        ...
        $listtindakan = json_decode($request->arrItem);

        $autoNumber = autoNumberTrans::autoNumber('TR-'.$tgl.'-', '4', false);

        $i = 0;

        foreach($listtindakan as $data){
            ...
            ${'inaptrans'.$i}->TransKelompok           = 'TMS';
            ${'inaptrans'.$i}->TarifJenis              = $data->tarifvarkel;
            ${'inaptrans'.$i}->TransTanggal            = $nowDate;
            ${'inaptrans'.$i}->TransKeterangan         = 'Tindakan Medis '.$data->tarifnama.' : '.$data->pelakunama;
            ${'inaptrans'.$i}->TransDebet              = 'D';
            ${'inaptrans'.$i}->TransBanyak             = floatval(str_replace(',', '', $data->banyak));
            ...
            if(${'inaptrans'.$i}->save()){
                // ========================= simpan ke tlogbook ==============================
                    ...
                    $logbook->TLogBook_LogMenuNo        = '04002';
        ...
        \DB::commit();
        session()->flash('message', 'Tindakan Medis Rawat Inap Berhasil Disimpan');

        return redirect('/tindakanmedisinap');

    }

    public function show($id)
    {
        return View::make('Rawatinap.Tindakanmedis.home');
    }
    ...
    public function edit($id)
    {
        ...
        return view::make('Rawatinap.Tindakanmedis.edit', compact('autoNumber', 'units', 'pelakus', 'admvars', 'tarifvars', 'provinsi', 'prsh', 'listtindmedis', 'rawatinaps'));  
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::commit();
        session()->flash('message', 'Edit Tindakan Medis Rawat Inap Berhasil');

        return redirect('/tindakanmedisinap/show');
    }
