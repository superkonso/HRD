...
class StockopnameController extends Controller
{
    ...
    {
        $this->middleware('MenuLevelCheck:04,008');
    }
    ...
    public function index()
    {
        ...
        $units      = DB::table('tunit')->where(DB::raw('substring("TUnit_Kode", 1, 2)'), '=', '05')
                        ->orderby('TUnit_Kode','ASC')->get();            
        $grups      = Grup::where('TGrup_Jenis', '=', 'OBAT')->orderBy('TGrup_Nama', 'ASC')->get();
        $unitnama   = 'Rawat Inap';
        $link       ='/stockopnameinap';
      
        return view::make('Rawatinap.Stokopname.create', compact('autoNumber', 'units', 'grups','unitnama','link'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // ============================================= validation ==================================
            ...
            if(empty($request->nomortrans) || $request->nomortrans == ''){
                ...
                return redirect('/stockopnameinap');
                exit();
            }
        // ============================================================================================
            ...
            if($opname->save()){
                ...
                if($obatrngkartu->save()){
                    // =============================== Simpan ke tstockmovingavg =============
                        ...
                        $stockmovingAVG->TStockMovingAVG_TransKeterangan    = 'Stock Opname Inap No. '.$autoNumber;
        ...
        $logbook->TLogBook_LogMenuNo    = '04008';
        $logbook->TLogBook_LogMenuNama  = url()->current();
        $logbook->TLogBook_LogJenis     = 'C';
        $logbook->TLogBook_LogNoBukti   = $autoNumber;
        $logbook->TLogBook_LogKeterangan = 'Stock Opname Inap No : '.$autoNumber .'-'. $data->kode;
        $logbook->TLogBook_LogJumlah    = floatval($data->subtotal);
        $logbook->IDRS                  = '1';

        if($logbook->save()){
            \DB::commit();
            session()->flash('message', 'Transaksi Stock Opname Unit Inap Berhasil Disimpan');
        }
    ...
    {
         return view::make('Rawatinap.Stokopname.home');
    }
    ...
    public function edit($id)
    {
        ...
        return view::make('Rawatinap.Stokopname.edit', compact('opnames','opnamesList', 'units', 'grups','unitnama','link'));
    }
    ...
    public function update(Request $request, $id)
    {
            ...
            if($opname->save()){
                ...
                if($obatrngkartu->save()){
                    // =============================== Simpan ke tstockmovingavg =============
                        ...
                        $stockmovingAVG->TStockMovingAVG_TransJenis         = 11;
                        $stockmovingAVG->TStockMovingAVG_AutoNumber         = $i;
                        $stockmovingAVG->TStockMovingAVG_TransKeterangan    = 'Stock Opname Inap No. '.$request->nomortrans;
                        $stockmovingAVG->TStockMovingAVG_TRDebet            = ($selisih > 0 ? abs($selisih) : 0);
                        $stockmovingAVG->TStockMovingAVG_TRKredit           = ($selisih < 0 ? abs($selisih) : 0);
                        $stockmovingAVG->TStockMovingAVG_Saldo_All          = 0;
                        $stockmovingAVG->TStockMovingAVG_Saldo_WH           = $qtyAkhir;
        ...
        $logbook->TLogBook_LogKeterangan = 'Stock Opname Unit No : '.$request->nomortrans .'-'. $data->kode;
        $logbook->TLogBook_LogJumlah    = floatval($data->subtotal);
        $logbook->IDRS                  = '1';
        ...
        return redirect('/stockopnameinap/show');
    }
