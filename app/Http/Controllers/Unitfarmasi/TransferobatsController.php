...
class TransferobatsController extends Controller
{
    public function __construct()
    {
        $this->middleware('MenuLevelCheck:06,103');
    }
    ...
    public function index()
    {
       ...
       date_default_timezone_set("Asia/Bangkok");
        ...
        return view::make('Unitfarmasi.Transferobat.create', compact('autoNumber','units'));
   ...
   public function create()
    ...
    public function editTransfer($kdUnit)
    {   
        $unit       = Unit::where('TUnit_Kode', '=', $kdUnit)->first();
        $unitnama   = ucwords(strtolower($unit->TUnit_Nama));
        
        return view::make('Unitfarmasi.Transferobat.home', compact('kdUnit','unitnama'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $mutasiobat->TUnit_Kode_Asal         = '031';
        ...
        if ($mutasiobat->save()){
            ...
            // ========================= simpan ke tlogbook ==============================
                ...
                $logbook->TLogBook_LogMenuNo    = '06103';
                $logbook->TLogBook_LogMenuNama  = url()->current();
                $logbook->TLogBook_LogJenis     = 'C';
                $logbook->TLogBook_LogNoBukti   = $autoNumber;
                $logbook->TLogBook_LogKeterangan = 'Transfer Obat : '.$mutasiobat->TObatMts_Nomor;
        ...
        return $this->ctktrfobat($autoNumber);
    ...
    public function edit($id)
    {
        ...
        $transferobats         = Obatmts::where('TObatMts_Nomor','=',$id)->first();       

        $tranferdetils    = DB::table('tobatmtsdetil as g')
                            ...
                            ->where('TObatMts_Nomor', '=', $transferobats->TObatMts_Nomor)->get();

        $tgl        =  date('Y-m-d H:i:s');

        foreach($tranferdetils as $data){
            $CekStok = saldoAkhirObat::hitungSaldoAkhirObat( $tgl , $transferobats->TUnit_Kode_Asal, $data->TObat_Kode, '');
            $stokQty = 0;
            $stokJml = 0;   
        ...
        $units              = Unit::where('TUnit_Kode', '=', $transferobats->TUnit_Kode_Tujuan)->get();

        $unit               = Unit::where('TUnit_Kode', '=', $transferobats->TUnit_Kode_Tujuan)->first();

        $unitnama           = ucwords(strtolower($unit->TUnit_Nama));

        return view::make('Unitfarmasi.Transferobat.edit', compact('transferobats','tranferdetils','unitnama','units'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $mutasiobat      = Obatmts::where('TObatMts_Nomor','=',$id)->first();
        ...
        if ($mutasiobat->save()){
            ...
            // ===========================================================================
        return $this->ctktrfobat($obatkmrno);
        
    }

    public function ctktrfobat($nomorretur) 
    {   
        $transferobat  = Obatmts::where('TObatMts_Nomor','=',$nomorretur)->first();
        return view::make('Unitfarmasi.Transferobat.cetaktransfer', compact('nomorretur','transferobat'));
    }
