...
class ObatstopoperasiController extends Controller
{
    public function __construct()
    {
        $this->middleware('MenuLevelCheck:09,003');
    }
...
public function index()
    {
        ...
        $units      = Unit::whereIn('TUnit_Kode', array('037'))->first();
        $grups      = Grup::where('TGrup_Jenis', '=', 'OBAT')->orderBy('TGrup_Nama', 'ASC')->get();
        $unitnama   = $units->TUnit_Nama;
        $link       ='/obatopso';

        return view::make('Kamaroperasi.Stockopname.create', compact('autoNumber', 'units', 'grups','unitnama','link'));
    }
    ...
    public function store(Request $request)    {
        ...
        $logbook->TLogBook_LogMenuNo    = '09003';
        $logbook->TLogBook_LogMenuNama  = url()->current();
        $logbook->TLogBook_LogJenis     = 'C';
        $logbook->TLogBook_LogNoBukti   = $autoNumber;
        $logbook->TLogBook_LogKeterangan = 'SO Kamar Operasi No : '.$autoNumber .'-'. $data->kode;
        $logbook->TLogBook_LogJumlah    = floatval($data->subtotal);
        $logbook->IDRS                  = '1';

        if($logbook->save()){
            \DB::commit();
            session()->flash('message', 'Transaksi Stock Opname Kamar Operasi Berhasil Disimpan');
        }
    ...
    public function show()    {   
        $kdUnit     = '037';
        $unitnama   = 'Kamar Operasi';
        $link       ='/obatopso';

        return view::make('Kamaroperasi.Stockopname.home', compact('kdUnit','unitnama','link'));
    }
    ...
    public function edit($id)
    {
        ...
        $units      = Unit::whereIn('TUnit_Kode', array('037'))->get();
        $grups      = Grup::where('TGrup_Jenis', '=', 'OBAT')->orderBy('TGrup_Nama', 'ASC')->get();
        $unitnama   = 'Kamar Operasi';
        $link       ='/obatopso';

        return view::make('Kamaroperasi.Stockopname.edit', compact('opnames','opnamesList', 'units', 'grups','unitnama','link'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($logbook->save()){
            \DB::commit();
            session()->flash('message', 'Edit Stock Opname Kamar Operasi Berhasil Disimpan');
        }
