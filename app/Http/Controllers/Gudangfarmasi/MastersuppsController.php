...
{   
    ...
    {
        $this->middleware('MenuLevelCheck:05,603');
    }
   
    public function index()
    {
        return view::make('Gudangfarmasi.Supplier.home');
    }
    ...
    public function create()
    {
        ...
        $autoNumber = autoNumberTrans::autoNumber('FAR-', '3', false);
      
        return view::make('Gudangfarmasi.Supplier.create', compact('autoNumber'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($supplierbaru->save())
        {
                ...
                $autoNumber = autoNumberTrans::autoNumber('FAR-', '3', true);

                $logbook->TUsers_id             = (int)Auth::User()->id;
                $logbook->TLogBook_LogIPAddress = $ip;
                $logbook->TLogBook_LogDate      = date('Y-m-d H:i:s');
                $logbook->TLogBook_LogMenuNo    = '603';
                $logbook->TLogBook_LogMenuNama  = url()->current();
                $logbook->TLogBook_LogJenis     = 'C';
                $logbook->TLogBook_LogNoBukti   = $autoNumber;
                $logbook->TLogBook_LogKeterangan = 'Supplier Baru '.$request->nama;
    ...
    public function edit($id)
    {
        ...
        return view::make('Gudangfarmasi.Supplier.edit', compact('supps'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($supplierbaru->save())
        {
                ...
                $logbook->TLogBook_LogKeterangan = 'Edit Supplier '.$request->nama;
                $logbook->TLogBook_LogJumlah    = '0';
                $logbook->IDRS                  = '1';

                if($logbook->save()){
                    \DB::commit();
                    session()->flash('message', 'Edit Data Supplier Berhasil');
                }
            // ===========================================================================
        }
